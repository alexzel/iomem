...
const { buildPacket, parsePacket, DEFAULT_EXTRAS } = require('../src/packet')

describe('packet', () => {
  it('builds and parses packet with key, value, and extras', () => {
    const packet = buildPacket(0x81, 0x01, 'testkey', 'testvalue', Buffer.from([1, 2, 3]), 0x02, 0x03, 0x04)

    const data = parsePacket(packet)
    expect(data).toBeTruthy()

    const [magic, opcode, key, value, extras, status, opaque, cas] = data
    ...
    expect(opaque).toBe(0x03)
    expect(cas).toBe(0x04)
  })

  it('builds and parses packet without key, value, and extras', () => {
    const packet = buildPacket(0x81, 0x01, '', '', DEFAULT_EXTRAS, 0x02, 0x03, 0x04)
  ...
  it('returns null when parsing packet with incorrect magic', () => {
    const packet = buildPacket(0x82, 0x01, '', '', DEFAULT_EXTRAS, 0x02, 0x03, 0x04)
    const data = parsePacket(packet)
    expect(data).toBe(null)
  })

  it('returns null when parsing packet with incorrect body size', () => {
    let packet = buildPacket(0x81, 0x01, '', '', DEFAULT_EXTRAS, 0x02, 0x03, 0x04)
